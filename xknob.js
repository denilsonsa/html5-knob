if (!window.XKnob) {
  (function () {
    ...
    const float_or_default = function (x, def) {
      x = parseFloat(x);
      return isNaN(x) ? def : x;
    };
    const int_or_default = function (x, def) {
      x = parseInt(x, 10);
      return isNaN(x) ? def : x;
    };
    ...
    const drag_rotate = function (ev) {
      ...
      const new_angle = xknob_being_dragged._get_mouse_angle(ev);
      const old_angle = xknob_drag_previous_angle;
      ...
      console.assert(delta_angle >= -Math.PI && delta_angle <= Math.PI, { delta_angle, old_angle, new_angle });

      const delta_value = delta_angle / Math.PI / 2;
      const new_proposed_value = xknob_drag_previous_value + delta_value;
      const old_actual_value = xknob_being_dragged.value;
      ...
      const new_actual_value = xknob_being_dragged.value;
    ...
    const keypress_handler = function (ev) {
      ...
      const initial_value = ev.target.value;
    ...
    class XKnob extends HTMLElement {
      connectedCallback() {
        ...
        for (const attr of ['divisions', 'min', 'max', 'svgsymbolid', 'value']) {
      ...
      _update_gfx_rotation () {
        if (this.shadowRoot) {
          const elem = this.shadowRoot.querySelector('.knob_gfx');
          if (elem) {
      ...
      _get_center_position () {
        ...
        const rect = this.shadowRoot.querySelector('.knob_center').getBoundingClientRect();
        return [
      ...
      _get_mouse_angle (ev) {
        const center = this._get_center_position();
